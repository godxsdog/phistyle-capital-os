    ...
    <main><div className="shell">
      ...
      <form className={`${styles.questForm} panel`} onSubmit={submitQuest}>
        ...
        <div className={styles.optionRow}>
          <label><span>旅程天數</span><input value={prefs.tripDays} onChange={(event) => setPrefs({ ...prefs, tripDays: event.target.value })} inputMode="numeric" required /></label>
        ...
        <p className="subtle">回程容忍旅程天數 ±1 天；同一查詢當日重跑會使用既有快照，節省 Pro 額度。</p>
      </form>

      <section className="panel">
        <div className="stage-header"><div><h2>尋票結果</h2>{activeQuest ? <p className="subtle">{activeQuest.origin} ⇄ {activeQuest.destination} · {activeQuest.window_start}～{activeQuest.window_end}</p> : null}</div><span className="stage-pill">總哩程優先</span></div>
        {results.length === 0 ? <p className="pending-text">尚無配對結果。輸入條件後開始尋票。</p> : <div className={styles.resultList}>{results.map((result) => <ResultCard key={result.id} result={result} fxRates={fxRates} promoted={promotedIds.includes(result.id)} onPromote={() => void promote(result)} />)}</div>}
      </section>

      <section className="panel"><h2>過往查詢</h2><div className={styles.questHistory}>{quests.map((quest) => <button className="button" key={quest.id} type="button" onClick={() => void openQuest(quest)}>#{quest.id} {quest.origin} ⇄ {quest.destination} · {quest.window_start}～{quest.window_end}</button>)}{quests.length === 0 ? <p className="pending-text">尚無查詢紀錄。</p> : null}</div></section>
    </div></main>
  );
}
