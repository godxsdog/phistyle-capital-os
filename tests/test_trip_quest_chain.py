...
def test_chain_requires_all_segments_same_day_filters_seats_and_ranks_verified_first():
    ...
    assert [row.outbound_date.isoformat() for row in first.results] == ["2026-11-01", "2026-11-04", "2026-11-05"]
    assert [str(row.total_miles) for row in first.results] == ["28000", "31000", "35000"]
    assert [json.loads(row.raw_refs)["bucket_verified"] for row in first.results] == [True, True, False]
    ...
    assert client.trip_calls == ["s1-d4", "s2-d4", "s1-d1", "s2-d1", "s1-d5", "s2-d5"]
